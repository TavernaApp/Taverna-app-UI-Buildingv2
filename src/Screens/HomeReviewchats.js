...
const Reviewchats = ({ navigation, userData }) => {
  ...
  const toggleLike = () => {
    ...
    setTimeout(() => {
      setLiked(false);
    }, 1000); // Change to the desired duration in milliseconds (1000ms = 1 second)
  };
  ...
  const onRefresh = async () => {
    ...
    // Function to render dollar signs based on priceTier
    const renderDollarSigns = (priceTier) => {
      return Array.from({ length: Math.min(priceTier, 3) }, (_, index) => (
        <Icon key={index} name="dollar" size={15} color="white" />
      ));
    };
  return (
    <LinearGradient
                ...
                <View style={styles.texttouchcontainer}>
                  {editReviewId === review.id ? (
                    <View style={{padding:3}}>
                      ...
                      <TouchableOpacity onPress={handleEditReview} style={{color:"white", marginLeft:5}}>
                        <Text style={{color:"white"}}>Save</Text>
                      </TouchableOpacity>
                      <TouchableOpacity onPress={() => setEditReviewId(null)} style={{color:"white", marginLeft:10}}>
                ...
                <View style={styles.upperbottomcontainer}>
                  <View style={styles.uforium}>
                    {isReplying && replyParentId === review.id ? (
                      // Render input field and send button when replying
                      ...
                      // Render 'Reply' button when not replying
                      <TouchableOpacity onPress={() => {
                        setIsReplying(true);   // Set isReplying to true to show reply input
                        setReplyParentId(review.id); // Set replyParentId to current review id
                      }}>
                    ...
                    {!isReplying && (
                      // Render heart button and additional elements if not replying
                      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                        <TouchableOpacity onPress={toggleLike}>
                          {/* Toggle heart icon based on 'liked' state */}
                          <Icon name={liked ? "heart" : "heart-o"} size={15} color={liked ? 'red' : 'silver'} />
                        </TouchableOpacity>
                        {/* Additional elements */}
                        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-evenly', marginLeft: '8%', width: '66%' }}>
                          <StarRatingCenter rating={review?.ratings[0]?.rating}/>
                      ...
                      {renderDollarSigns(review.ratings[0].priceTier || 0)} 
                      </View>
                          <Icon name="heart" size={15} />
                          <Text>{review.ratings[0]?.likes}</Text>
                        </View>
  ...
  placeholder={editReviewId !== null ? "Edit your reply..." : "Write a reply..."} // Conditional placeholder
  onChangeText={setEditReviewText} // For editing or replying to an existing review
  value={editReviewText} // Holds the text for reply or edit
/>
...
export default connect(mapStateToProps)(Reviewchats);
